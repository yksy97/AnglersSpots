...
# -「Post」モデルが他のモデルと関連付けされているからこそ、このメソッドが重要。このメソッドが指定されていない場合、Ransackで特定のモデルに関連付けられている他のモデルを検索対象にできない。
...
# 82行目「before_update :update_rigs」
# -このコールバックは、Postモデルのレコードが更新される直前に実行されるコールバック。
# 実行時、フォームから送信された新しいリグのリスト（self.rig.list）を使用して、現在の投稿に紐つけられているリグ（rigs）の更新を行う。
# -更新とは、①新しいリグのリストに含まれているが、現在の投稿に紐つけていないリグは、新しく投稿に紐付ける。　②現在の投稿に紐付いているが、新しいリグリストに含まれていないリグは、投稿の紐付けから削除する。
